...
export async function POST(req: Request) {
	try {
		...
		try { await fs.access(scriptPath); } catch { return NextResponse.json({ error: 'Backup script missing' }, { status: 500 }); }
		...
		if (snapMatch) {
			...
			const createLine = output.find(l => l.includes('Creating snapshot at')) || '';
			...
			let archived = false;
			if (snapshot) {
				const manifestPath = path.join(projectRoot, 'backups', snapshot, 'MANIFEST.txt');
				try {
					const manifest = await fs.readFile(manifestPath, 'utf8');
					const m = manifest.match(/Total Files: (\d+)/); if (m) files = parseInt(m[1], 10);
				} catch { /* ignore */ }
				const archivePath = path.join(projectRoot, 'backups', snapshot + '.tar.gz');
				archived = await fs.access(archivePath).then(() => true).catch(() => false);
			}
			return NextResponse.json({ success: true, archive, archived, tag, snapshot, files, durationMs, raw: output.join('') });
	} catch (e: any) {
